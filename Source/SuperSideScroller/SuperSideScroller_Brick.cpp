...
ASuperSideScroller_Brick::ASuperSideScroller_Brick()
{
	...
	BrickMesh->SetCollisionProfileName(TEXT("BlockAll"));
	RootComponent = BrickMesh;

	BrickCollision = CreateDefaultSubobject<UBoxComponent>(TEXT("BrickCollision"));
	BrickCollision->SetCollisionProfileName(TEXT("BlockAll"));
	BrickCollision->AttachToComponent(RootComponent, FAttachmentTransformRules::KeepRelativeTransform);
...
void ASuperSideScroller_Brick::PlayHitExplosiion()
{
	UWorld* World = GetWorld();
	if (World && Explosion)
	{
